...
def get_files(file_dir):
    ...
    for file in os.listdir(file_dir):
        name = file.split(sep='.')
        if 'lsw' in name[0]:
            lsws.append(file_dir + file)
            label_lsws.append(0)
        else:
            if 'pouch' in name[0]:
                pouchs.append(file_dir + file)
                label_pouchs.append(1)
        image_list = np.hstack((lsws, pouchs))
        label_list = np.hstack((label_lsws, label_pouchs))
...
# 测试 get_files
# imgs , label = get_files('E:/lswVSpouch/data/train/')
# for i in imgs:
# 	print("img:",i)
#
# for i in label:
...
# IMG_H = 208
#
# train_dir = 'E:/lswVSpouch/data/train/'
#
